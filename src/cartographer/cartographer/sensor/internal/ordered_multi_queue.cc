    ...
    */
    ...
    for (auto it = queues_.begin(); it != queues_.end();) {
      ...
      if (data == nullptr) {
        ...
        // 退出条件1: 某个话题的数据队列为空同时又不是完成状态, 就先退出,
        // 发布log并标记为阻塞者
    ...
    // 如果我们还没有为这个轨迹分配任何数据, 快进这个轨迹的所有队列,
    // 直到达到一个共同的开始时间

    // Step: 2 获取对应轨迹id的所有数据队列中的最小共同时间戳,
    // 作为轨迹开始的时间
    ...
    // 数据时间小于common_start_time,同时数据队列数据的个数小于2,只有1个数据的情况
    // 罕见
    else if (next_queue->queue.Size() < 2) {
      // 退出条件3: 数据队列数据的个数少,又不是完成状态,
      // 不能确定现在到底是啥情况, 就先退出稍后再处理
...
void OrderedMultiQueue::CannotMakeProgress(const QueueKey& queue_key) {
  ...
  for (auto& entry : queues_) {
    ...
    if (entry.second.queue.Size() > kMaxQueueSize) {
      ...
      // [ WARN] [1628516438.493835120, 1606808659.273453929]: W0809
      // 21:40:38.000000 10662 ordered_multi_queue.cc:230] Queue waiting for
      // data: (0, points2)
      // [ WARN] [1628516439.089736487, 1606808659.869309184]: W0809
      // 21:40:39.000000 10662 ordered_multi_queue.cc:230] Queue waiting for
      // data: (0, points2)
      return;
    }
  }
...
common::Time OrderedMultiQueue::GetCommonStartTime(const int trajectory_id) {
  ...
  // 成员变量 second 是个布尔值, 表示是否插入成功,
  // 如果这个元素的索引已经存在插入会失败,返回false
  ...
  if (emplace_result.second) {
    ...
    // [ INFO] [1628516134.243770381, 1606808649.533687125]: I0809
    // 21:35:34.000000  8604 ordered_multi_queue.cc:264] All sensor data for
    // trajectory 0 is available starting at '637424054495384530'.
  }
