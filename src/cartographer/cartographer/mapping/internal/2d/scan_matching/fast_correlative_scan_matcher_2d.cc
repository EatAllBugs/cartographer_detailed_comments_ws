...
PrecomputationGrid2D::PrecomputationGrid2D(
  ...
  for (int y = 0; y != limits.num_y_cells; ++y) {
    ...
    // intermediate的索引x + width - 1 + y * stride的范围是 [0, width-2] 再加上
    // y * stride
    ...
    // x + width - 1 + y * stride 的范围是 [width-1, limits.num_x_cells-2]
    // 再加上 y * stride
    ...
    // Step: 3 滑动窗口正在划出, 一次减少一个值, 所以intermediate的宽度比grid多
    // width-1
    // x + width - 1 + y * stride 的范围是 [limits.num_x_cells-1,
    // limits.num_x_cells+width-1] 再加上 y * stride
    ...
    // 理论上, 滑窗走完地图的一行之后应该是空的, 经过 只入, 一出一入, 只出,
    // 3个步骤
  ...
  // 根据intermediate的值, 对每一列从下到上竖着再做一次滑窗,
  // 这个才是真正的地图cells_
...
PrecomputationGridStack2D::PrecomputationGridStack2D(
  ...
  // 分辨率逐渐变大, i=0时就是默认分辨率0.05, i=6时,
  // width=64,也就是64个格子合成一个值
...
bool FastCorrelativeScanMatcher2D::MatchWithSearchParameters(
  ...
  // 缩小搜索窗口的大小,
  // 计算每一帧点云在保证最后一个点能在地图范围内时的最大移动范围
  ...
  // 检查最优解的值,
  // 如果大于指定阈值min_score就认为匹配成功,否则认为不匹配返回失败
...
FastCorrelativeScanMatcher2D::GenerateLowestResolutionCandidates(
  ...
  // 将所有候选解保存起来, 候选解的结构为（角度的索引, x偏移量, y偏移量,
  // 搜索参数）
...
void FastCorrelativeScanMatcher2D::ScoreCandidates(
  ...
  for (Candidate2D& candidate : *candidates) {
    ...
    // 栅格值的和除以这个点云中点的个数, 作为这个候选解在这个
    // precomputation_grid 上的得分
...
Candidate2D FastCorrelativeScanMatcher2D::BranchAndBound(
  ...
  for (const Candidate2D& candidate : candidates) {
    //  Step: 剪枝 低于设置的阈值 或者 低于上一层的可行解的最高分
    //  的可行解不进行继续分枝
    // 如果遇到一个候选点的分低于阈值,
    // 那么后边的候选解的得分也会低于阈值,就可以直接跳出循环了
