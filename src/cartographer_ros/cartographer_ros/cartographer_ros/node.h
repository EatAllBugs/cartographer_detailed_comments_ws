...
class Node {
 public:
  ...
  // c++11: =delete: 禁止编译器自动生成默认函数; =default:
  // 要求编译器生成一个默认函数
 ...
 private:
  ...
  // c++11: GUARDED_BY 是在Clang Thread Safety
  // Analysis（线程安全分析）中定义的属性
  ...
  // 相对于std::map, unordered_map的查找和插入的效率高,
  // 时间复杂度为常数级别O(1),而额外空间复杂度则要高出许多
  // 对于需要高效率查询的情况, 使用unordered_map容器,
  // 但是unordered_map对于迭代器遍历效率并不高
